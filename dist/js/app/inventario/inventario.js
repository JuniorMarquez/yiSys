...
app.controller('InventarioCtrl', ['$scope', '$state','$http', '$filter', '$modal', 'MyService', 'filterFilter', 'toaster','$timeout',  function($scope,  $state ,$http, $filter,$modal, MyService, filterFilter, toaster,$timeout) {
 $scope.nivel=MyService.data.nivel;
  ...
  $scope.carga = function (){
    $http.get('http://localhost:1337/categoria/?idEstablecimiento='+MyService.data.idEstablecimiento).then(function (resp) {
...
// };
  ...
  $http.get('http://localhost:1337/departamento/?idEstablecimiento='+MyService.data.idEstablecimiento).then(function (resp) {
...
$scope.openPeso = function (item) {
  ...
  $scope.deleteCategoria = function(item){
    $http.delete('http://localhost:1337/categoria/'+item.id , item)
    $scope.categorias.splice($scope.categorias.indexOf(item), 1);
  };
  ...
  $scope.selectItem = function(item){    
    ...
    //  $http.get('http://localhost:1337/peso/?idArticulo='+identificador).then(function (resp) {
  ...
  $scope.deleteArticulo = function(articulo){
    $http.delete('http://localhost:1337/articulo/'+articulo.id , articulo)
  ...
  $scope.doneEditingCategoria = function(item){
    ...
    if (MyService.data.idenCategoria){
      $http.put('http://localhost:1337/categoria/'+MyService.data.idenCategoria, categoriaAct)
    }
    else {
      $http.post('http://localhost:1337/categoria/', categoriaAct)
    }
    // $http.get('http://localhost:1337/categoria/?idEstablecimiento='+MyService.data.idEstablecimiento).then(function (resp) {
    //   $scope.categorias = resp.data.results;
    // });
  ...
  articuloAct.estado=item.estado;
   articuloAct.control=false;
    ...
    if (MyService.data.idenArticulo){
      $scope.pop4();
      $http.put('http://localhost:1337/articulo/'+MyService.data.idenArticulo , articuloAct)
    }
    else {
      $scope.pop3();;
      $http.post('http://localhost:1337/articulo/', articuloAct)
    }
    $http.get('http://localhost:1337/categoria/?idEstablecimiento='+MyService.data.idEstablecimiento).then(function (resp) {
      $scope.categorias = resp.data.results;
    });
